...
static gboolean gst_rtp_h264_depay_packet_lost (GstRTPBaseDepayload * depay,
    GstEvent * event);
static void gst_rtp_h264_depay_request_keyframe (GstRtpH264Depay * rtph264depay);
...
gst_rtp_h264_set_src_caps (GstRtpH264Depay * rtph264depay)
{
  ...
  if (old_caps != NULL) {
     ...
     *
     * @mkikot: Always update caps to make sure updated SPS
     * makes it into the downstream element
     */
